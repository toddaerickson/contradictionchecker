...
- The predicate lives in `consistency_checker/corpus/ocr.py::needs_ocr` and takes the fast-path element list plus the file path. It returns `True` only when fast extracted essentially no body text AND the file is a PDF whose page count is non-trivial. No side effects, no I/O beyond `len(elements)` and a `pypdf` page count — pure enough to unit-test without fixtures.
...
- Every escalation writes an audit row to `<data_dir>/ocr_events.jsonl` via `OcrAudit` (also in `ocr.py`). The row records the file path, the fast-path element count, the post-escalation element count, wall-clock duration, and the outcome. This is the surface that lets a future reviewer answer "did OCR fire on this document and what did it produce?" without re-running ingest.
