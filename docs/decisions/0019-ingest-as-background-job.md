...
- **Failure keeps the corpus.** A failed ingest no longer rolls the corpus away; it stays with a visible `failed` run so the user can see *why*. (This makes a **Delete corpus** affordance necessary — tracked as the next CRUD step; `store.delete_corpus()` already exists but is unexposed.)
