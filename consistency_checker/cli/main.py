...
def report(
    ...
    text = render_report(store, audit_logger, run_id=target_run)
    if out is None:
...
@app.command(
    help=(
        "Mine reviewer_verdicts for per-detector precision. Reads the audit DB only; no LLM calls."
    )
)
def eval(
    config: Path = typer.Option(Path("config.yml"), "--config", "-c"),
    out: Path | None = typer.Option(
        ...
        "-o",
        help=(
            "Optional output directory. When set, writes cc_eval_precision_<ts>.csv "
